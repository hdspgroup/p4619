...
def train_pinn(
    ...
    def _train_joint_step(dyn_weights, t, physics_ramp=1.0, do_step=True):
        """
        ...
        def _compute_data_loss():
            ...
            return loss_data_u, final_pred, data_target

        loss_data_u, pred_data, true_data = _compute_data_loss()
        ...
        loss_total = (dyn_weights["data_u"] * base_weights["data_u"] * loss_data_u +
                      physics_ramp * dyn_weights["pde"] * base_weights["pde"] * loss_pde +
                      physics_ramp * dyn_weights["bc"] * base_weights["bc"] * loss_bc +
                      physics_ramp * dyn_weights["flux"] * base_weights["flux"] * loss_flux +
        ...
        _check_numerical_stability(
            {"loss_total": loss_total, "loss_data_u": loss_data_u, "loss_pde": loss_pde, "loss_bc": loss_bc, "loss_flux": loss_flux},
        ...
        return loss_total, loss_data_u, loss_pde, loss_bc, loss_reg, loss_flux, gn_u_pre, gn_u_post, gn_s_pre, gn_s_post, u_layer_grads, sigma_layer_grads, rmse_val, corr_val
    ...
    for epoch in pbar_adam:
        ...
        epoch_losses = {k: 0.0 for k in ["total", "data_u", "pde", "bc", "reg", "flux"]}
        epoch_rmse = 0.0
        epoch_corr = 0.0
        ...
        for batch in pbar_batches:
            ...
            physics_ramp = min(1.0, (epoch + 1) / max(warmup_epochs, 1))
            res = _train_joint_step(dyn_weights, tensors, physics_ramp=physics_ramp)
            loss_total, loss_data_u, loss_pde, loss_bc, loss_reg, loss_flux, gn_u_pre, gn_u_post, gn_s_pre, gn_s_post, u_layer_grads, sigma_layer_grads, rmse_val, corr_val = res
        ...
        pbar_adam.set_postfix(
            ...
            rmse_V=f"{epoch_rmse:.3e}",
            corr_V=f"{epoch_corr:.3f}",
            phys=f"{physics_ramp:.2f}",
            lr=f"{current_lr:.2e}",
        )
    ...
    if num_epochs_lbfgs > 0:
        ...
        for _ in tqdm(range(num_epochs_lbfgs), desc="LBFGS"):
            def closure():
                ...
                result = _train_joint_step(dyn_weights, lbfgs_tensors, physics_ramp=1.0, do_step=False)
                return result[0]
