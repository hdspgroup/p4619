...
outDir = fullfile(rootDir, 'outputs', 'map_ec_to_soil_properties_demo');
if ~exist(outDir, 'dir')
    mkdir(outDir);
end

ecOnlyFile = fullfile(rootDir, 'outputs', 'ert_ec_only_inversion_demo', 'ert_ec_only_inversion_demo.mat');
if ~isfile(ecOnlyFile)
    error('EC-only inversion result not found: %s', ecOnlyFile);
end
...
if saveArg
    save(fullfile(outDir, 'map_ec_to_soil_properties_demo.mat'), ...
        'T', 'fieldsMapped', 'predFields', 'summary', 's');
    exportgraphics(fig, fullfile(outDir, 'map_ec_to_soil_properties_demo.png'), 'Resolution', 220);
end
