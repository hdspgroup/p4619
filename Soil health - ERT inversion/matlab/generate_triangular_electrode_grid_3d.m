...
% Practical quadrupole filters for a shallow-target planar array.
options = struct();
options.MinCurrentSpacing = spacing * 0.95;
options.MinPotentialSpacing = spacing * 0.95;
options.MinCentroidSeparation = spacing * 0.75;
...
arrayTag = sprintf('triangular_planar_s%.1f_r%d_td%.1f', spacing, round(maxRadius), targetDepth);
arrayTag = regexprep(arrayTag, '[^a-zA-Z0-9_.-]+', '_');

%% Electrode coordinates

electrodes = generate_triangular_planar_electrodes( ...
    'Spacing', spacing, ...
    'MaxRadius', maxRadius, ...
    'CenterXY', centerXY, ...
    'Z', electrodeZ, ...
...
summary.layoutType = "triangular_planar";
summary.spacing = spacing;
summary.maxRadius = maxRadius;
summary.centerXY = centerXY;
summary.rotationDeg = rotationDeg;
...
text(ax1, electrodes(:, 1) + 0.35, electrodes(:, 2) + 0.35, string(1:survey.nElectrodes), 'FontSize', 8, 'Color', [0.15 0.15 0.15]);
plot(ax1, centerXY(1), centerXY(2), 'k+', 'MarkerSize', 12, 'LineWidth', 1.2);
axis(ax1, 'equal');
grid(ax1, 'on');
xlabel(ax1, 'x (m)');
ylabel(ax1, 'y (m)');
title(ax1, sprintf('Triangular layout: %d electrodes, spacing %.1f m', survey.nElectrodes, spacing));
...
text(ax2, electrodes(:, 1) + 0.35, electrodes(:, 2) + 0.35, string(1:survey.nElectrodes), 'FontSize', 8, 'Color', [0.15 0.15 0.15]);
plot(ax2, centerXY(1), centerXY(2), 'k+', 'MarkerSize', 12, 'LineWidth', 1.2);
axis(ax2, 'equal');
grid(ax2, 'on');
xlabel(ax2, 'x (m)');
ylabel(ax2, 'y (m)');
title(ax2, sprintf('Coverage: %d ABMN, %d unused electrodes', survey.nMeasurements, survey.summary.unusedElectrodes));
cb = colorbar(ax2);
